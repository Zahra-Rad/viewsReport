...
// creating a 30-day sum for watch times and visits
...
// creating arrays from data groups

const total_visits_array = [];
const daily_visits_array = [];
const followers_count_array = [];
const daily_sub_array = [];
...
selector.addEventListener("click", () => {
  selector.addEventListener("change", () => {
    ...
    lineChart.data.labels = dates.slice(0, range).reverse();
    ...
    lineChart.data.datasets[2].data = followers_count_array
      .slice(0, range)
      .reverse();
    lineChart.data.datasets[3].data = daily_sub_array.slice(0, range).reverse();
    lineChart.update();
  });
});
...
let lineChart = new Chart(ctx, {
  type: "line",
  data: {
    labels: dates.slice(0, range).reverse(),
    datasets: [
      {
        label: "Total Visits",
        data: total_visits_array.slice(0, range).reverse(),
        borderWidth: 1,
      },
      {
        label: "Daily Visits",
        data: daily_visits_array.slice(0, range).reverse(),
        borderWidth: 1,
      },
      {
        label: "Followers",
        data: followers_count_array.slice(0, range).reverse(),
        borderWidth: 1,
      },
      {
        label: "Daily Subscription",
        data: daily_sub_array.slice(0, range).reverse(),
        borderWidth: 1,
      },
    ],
  },
